...
## Run s Stepwise Regression for variable selection
...
##A possible model after running the step wise regression is
##asking_price=31.572+0.109∗yearly_tax−0.551∗age+8.846∗brick−0.008∗living_space+ϵ

##Best Subset regression to select the variables

best_subset <- ols_step_best_subset(full_model, details = TRUE)
best_subset
...
## Asses the CLRM Assumptions
## Assess the Linearity


df <- data.frame(asking_price, living_space, yearly_tax, age, brick, lot_size, bedrooms)
plot(df)
...
final_model <- lm(asking_price ~ living_space + yearly_tax + age + brick + lot_size + bedrooms)
resi_final_model <- residuals(final_model)
fit_final_model <- predict(final_model)

##Plotting residual vs fitted values

plot(x = fit_final_model, y = resi_final_model,  main = "Residual vs Fitted values", xlab = "Fitted",ylab = "Residual")

## Plotting residual vs all X

## Residual vs living_space

plot(x = living_space, y = resi_final_model, main = "Residual vs Living Space", xlab = "Living Space", ylab = "Residual")

##Plotting a Residual vs yearly_tax

plot(x = yearly_tax, y = resi_final_model, main = "Residual vs Yearly Tax", xlab = "Yearly Tax", ylab = "Residual")

##Plotting a Residual vs age

plot(x = age, y = resi_final_model, main = "Residual vs Age", xlab = "Age", ylab = "Residual")

##Plotting a Residual vs fireplace

plot(x = fireplace, y = resi_final_model, main = "Residual vs Fireplace", xlab = "Fireplace", ylab = "Residual")

##Plotting a  Residual vs bedrooms
plot(x = bedrooms, y = resi_final_model, main = "Residual vs Bedrooms", xlab = "Fireplace", ylab = "Residual")

##Plotting a  Residual vs lot_size
plot(x = lot_size, y = resi_final_model, main = "Residual vs Lot size", xlab = "Lot Size", ylab = "Residual")

##As seen from the residual wrt the fitted and residual vs X plot, it can be inferred that there is presence of heteroscedasticity in the data which is introduced due to an  an outlier

##Assess Normality
##As seen from the residual vs fit plot, it can be seen that there is no apparent pattern in the data. Thus it can be inferred that the data is normal

##Remedy of  Heteroscedasticity
##As the outlier does not fall within any of the three cases to remedy heteroscedasticity, the log transform is implemented on the output variable as it can reduce heteroscedasticity

# Transform the variables
ln_asking_price <- log(asking_price)

# Runnthe regression on transformed variables
trans_model <- lm(ln_asking_price ~ living_space + yearly_tax + age + brick + lot_size + bedrooms)
resi_trans_model <- residuals(trans_model)
fit_trans_model <- predict(trans_model)

## Plot the residual square vs fitted values

plot(x = fit_trans_model, y = resi_trans_model, main = "Residual vs Fitted values", xlab = "Fitted", ylab = "Residual")
...
## Interpretating of  the final model
summary(trans_model)

##The estimated regression model is
## population=5.225e−2.777e−05∗living_space+2.349e−04∗yearly_tax−1.530e−03∗age+3.924e−02∗brick+4.135e−02∗lot_size−3.970e−03∗bedrooms+ϵ

## After running the Global F-Test
## The p-value is < 2.2e-16.Thus it can be concluded that the overall model is valid.
...
## After running the Individual T-test.
## The variable living space, age, yearly_tax and brick are significant at 0.05 significance level.

## Interpret the coefficients.
## Living Space:On increasing the living space by 1 sq feet decreases the asking price by 0.0027%, while keeping other variables constant.

## Yearly Tax: On increasing the yearly tax by 1 dollar increases the asking price by 0.0239%, while keeping other variables constant.

## Brick: In case the house is built-up of brick it increases the asking price by 3.92%, while keeping other variables constant.

## Age: With an increase in age of the house by 1 year decreases the asking price by 0.15% while keeping other variables constant.
...
## This study can be used by management to identify the rate of absenteeism that employees with a greater job experience and higher complexity of work are less dissatisfied or satisfied with their respective managers wrt to the ones that are not the experienced
## Hence leads to believe that the absenteeism rate is higher for the younger employees when compared to the experienced employees or older employees.
...
model <- lm(y ~ x)
summary(model)
## The equation is y = 8.806181 + 0.998917*x
...
### Obtain the residuals, square them, and take their logs
